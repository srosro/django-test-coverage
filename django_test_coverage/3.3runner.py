...
if PY3 >= int(3):
    ...
    print("this version only supports python 3.x.x and django 1.6 and above")
...
def _get_app_package(label):
    ...
    print(path)
    print('this is the imp')
    print(imp)
    print('this is the app')
    print(app)
    path_list = path.split(os.sep)
    #chop off models.py and call that the path list?
    ...
    if path_list[-1] == 'models':
        ...
        path_list.pop()
    path_list.reverse()
    print('this is the path list')
    print(path_list)
    for p in path_list:
        print('were in the path list')
        print('were printing p')
        imp.insert(0, p)
        try:
            pkg = __import__('.'.join(imp), {}, {}, [''])
            print('this is the pkg')
            print(pkg)
            return pkg, '.'.join(imp)
        except ImportError:
            continue
...
def _package_modules(pkg, impstr):
    ...
    modules = []
    print(pkg)
    #fixed syntax here to be python 3.3 compatible

    path = pkg.__path__[0]
    print("this is the path of the app")
    ...
    for f in files:
        ...
        if os.path.isdir(test_me) and f != '__pycache__':
            #subpackage
            print('we got to subpackage')
            print('this is an imp')
            imp = impstr+ '.' + f
            try:
                spkg = __import__(imp, {}, {}, [''])
